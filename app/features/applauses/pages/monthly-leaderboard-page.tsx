...
export const meta:Route.MetaFunction = ({params, data}) => {
    ...
    return [
        {title: `Best of ${date.toLocaleString({month:"long", year:"2-digit"})} | app_lause`}
    ]
}
...
export default function MonthlyLeaderboardPage({loaderData}: Route.ComponentProps) {
    ...
    return (
        <div className="space-y-10">
            <Hero title = {`Best of ${urlDate.toLocaleString({month:"long", year:"2-digit"})}`}/>
            <div className="flex items-center justify-center gap-2">
                <Button variant="secondary" asChild>
                    <Link to={`/applauses/leaderboards/monthly/${previousMonth.year}/${previousMonth.month}`}>&larr; {previousMonth.toLocaleString({month:"long", year:"2-digit"})}</Link>
                </Button>
            {!isToday ? (
                <Button variant="secondary" asChild>
                    <Link to={`/applauses/leaderboards/monthly/${nextMonth.year}/${nextMonth.month}`}>{nextMonth.toLocaleString({month:"long", year:"2-digit"})} &rarr;</Link>
                </Button>
                ) : null
